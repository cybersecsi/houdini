...
export const replaceHoudiniVariables = (initialRunCmd: string, currentToolbox: IToolbox[]) => {
  ...
  const inputValues = toolInputs.reduce((prev: {[inputName: string]: string}, curr: string ) => {
    const inputName = curr.replace(/<|>/g, '')
    const inputValue = currentToolbox.find((v: IToolbox) => v.inputName === inputName)?.value ?? curr
    return  {
      ...prev,
      [curr]: inputValue
    }
